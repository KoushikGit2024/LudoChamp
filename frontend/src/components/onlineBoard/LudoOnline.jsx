...
const LudoOnline = memo(({ socket, socketLoaded, setSocketLoaded, boardType }) => {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // BUG F5 FIX: verifySyncSequence had two wrong conditions:
  //   (a) `incomingTick === currentTick` returned true — a duplicate tick is
  //       NOT valid; the same tick arriving twice means we missed an update.
  //   (b) Fallthrough after the equality check never reached the mismatch
  //       branch correctly.
  //
  // Fixed logic: the ONLY valid incoming tick is exactly currentTick + 1.
  // Any other value (same, skipped, lower) triggers a resync.
  // ─────────────────────────────────────────────────────────────────────────
  const verifySyncSequence = (incomingTick) => {
    if (typeof incomingTick !== 'number') return true;  // No tick info — allow
    const currentTick = useGameStore.getState().meta.syncTick || 0;
    if (currentTick === 0) return true;                 // First event — allow

    if (incomingTick === currentTick + 1) return true;  // Expected — allow

    // Anything else is a desync
  ...
  useEffect(() => {
    ...
    // ─────────────────────────────────────────────────────────────────────
    // join-success: fired for POI players after the server assigns a color.
    // For POF this is intentionally skipped — state arrives via initiate-game.
    // ─────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────
    // initiate-game: fired when ALL players have joined a POF game.
    //
    // BUG F4 FIX: Previously myColor was never set for POF players.
    // handleJoinSuccess was guarded by `if (boardType !== "poi") return` and
    // handleInitiateGame never looked up the user's own color in the state.
    //
    // Fix: After syncing full state, scan serverState.players to find which
    // color belongs to this user (matched by userId === userInfo.username).
    // ─────────────────────────────────────────────────────────────────────
    const handleInitiateGame = ({ state }) => {
      ...
      // Set myColor for POF players who couldn't get it from join-success
      if (!myColorRef.current && userInfo?.username) {
        for (const c of ["R", "B", "Y", "G"]) {
          if (state.players[c]?.userId === userInfo.username) {
    ...
    const handleStateSynced = (serverState) => {
      ...
      // After a full resync, recover myColor if it was lost
      if (!myColorRef.current && userInfo?.username) {
        for (const c of ["R", "B", "Y", "G"]) {
  ...
  // BUG F6 FIX: Removed debug toast that was firing on every socketLoaded change.
  // Original: useEffect(()=>{ toast.info(`socketLoaded: ...`) },[socketLoaded])
