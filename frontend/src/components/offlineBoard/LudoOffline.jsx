...
const LudoOffline = () => {
  ...
  const {winR,winB,winY,winG}=useGameStore(useShallow(state=>{
    return{
      ...
      winB:state.players.R.winPosn,
      winY:state.players.R.winPosn,
      winG:state.players.R.winPosn,
    }
  }));
  ...
  return (
    ...
    >
      ...
      <div className='flex flex-row items-center justify-between min-w-full h-1/12'>
        ...
        >
          <Dice turn={turn} pieceIdx={pieceIdx} winPosn={winPosn} ticks={moveObj.ticks} homeCount={homeCount} rollAllowed={rollAllowed} moveAllowed={moveObj.moveAllowed} timeOut={timeOut} gameFinished={playerCount===winLast}  winState={winState}/>
          {/* {Number(screen)} */}
        </div>
        <PlayerBoard 
