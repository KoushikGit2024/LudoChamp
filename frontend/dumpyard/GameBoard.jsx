...
import SlideEffect from '../src/assets/SlideEffect.mp3'
import FinishSound from '../src/assets/FinishSound.mp3'
import gsap from "gsap";
// import {useGSAP} from '@gsap/react'
import debounce from '../src/derivedFuncs/debounce.js'
import Cell from "../src/components/sharedBoardComponents/Cell";
// import Room from "../../../../dumpyard/Room.jsx";
import useGameStore from '@/store/useGameStore'
import gameActions from '@/store/gameLogic'
import { useShallow } from "zustand/shallow";
import piecePath from "../src/contexts/PiecePath"
const GameBoard = memo(({moveCount,timeOut,moving,pieceIdxArr,winState,sound}) => {
  ...
  const updatePieceState=gameActions.updatePieceState;
  ...
  const oneStepAnimation = (from=0, to=0) => {
    return new Promise(resolve => {
      ...
      const curRect =chariotRef.current.getBoundingClientRect()
      console.log({x:curRect.left-boardRect.left})
...
// console.log('moveAllowed',moveAllowed); 
  ...
  return (
    ...
    >
      ...
      <div ref={chariotRef} className="piece aspect-square absolute z-100 bg-transparent bgg-[#cc1dcf] text-[10px] p-[2p] m-0 flex items-center justify-center bg-amber-400"  style={{width:`auto`,display:(`${(showChariot||1) ?"flex":"none"}`)}}>
        <Cell
          R={chariotColor==='R'} 
          B={chariotColor==='B'} 
          Y={chariotColor==='Y'} 
